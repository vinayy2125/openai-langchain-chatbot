...
class PromptBase(BaseModel):
    ...
    display_order: int = Field(default=0, description="The order in which to display the prompt")
...
class MessageBase(BaseModel):
    ...
    follow_up_to: Optional[str] = Field(None, description="ID of the message this is following up")
...
class FollowUp(BaseModel):
    ...
    context: str = Field(..., description="Context explaining why this follow-up is being asked")
    options: Optional[List[str]] = Field(default=None, description="Possible response options for nested questions")
...
class SentMessage(BaseModel):
    ...
    detailed: bool = Field(
        default=False,
        description="Whether to generate detailed responses"
    )
...
class StreamingChatResponse(BaseModel):
    status: str = Field(..., description="Status of the response: 'follow_up', 'complete', 'error'")
    message: str = Field(..., description="Main response message")
    follow_up: Optional[FollowUp] = Field(None, description="Follow-up question if status is 'follow_up'")
