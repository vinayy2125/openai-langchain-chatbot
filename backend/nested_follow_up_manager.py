...
Respond with ONLY: COMPLETE or CONTINUE"""
            ...
            logger.debug(f"[check_requirements] LLM evaluation after {meaningful_exchanges} exchanges: {evaluation}")
            logger.debug(f"[check_requirements] Will use {'comprehensive response' if is_complete else 'optimized response'}")
            return is_complete
...
"""
        ...
        messages = [
            {"role": "system", "content": "You are an AI assistant that provides concise (200 words max), focused responses with minimal formatting. Use only bullet points and bold text where absolutely necessary. Avoid unnecessary headers or complex formatting."},
...
"""
    ...
    def generate_follow_ups(self, session_id: str, latest_query: Optional[str] = None, context: Optional[str] = None) -> List[str]:
...
"""
        ...
        messages = [
            {"role": "system", "content": "You generate dynamic, open-ended follow-up questions based on conversation context."},
